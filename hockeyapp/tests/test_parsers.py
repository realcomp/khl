...
class HockeyAppParserTest(base.tests.BaseTest):
    ...
    def _check_parsers(self):
        ...
        self.khl_player_data = parsers.player.KHLPlayerInfo().get_page(self.khl_player_id)
        self.assertIsNotNone(self.khl_player_data)

        # test mhl player parser
        self.mhl_player_data = parsers.player.MHLPlayerInfo().get_page(self.mhl_player_id)
        self.assertIsNotNone(self.mhl_player_data)

        # test mhl2 player parser
        self.mhl2_player_data = parsers.player.MHL2PlayerInfo().get_page(self.mhl2_player_id)
        self.assertIsNotNone(self.mhl2_player_data)

        # test vhl player parser
        self.vhl_player_data = parsers.player.VHLPlayerInfo().get_page(self.vhl_player_id)
        self.assertIsNotNone(self.vhl_player_data)

        # test mhl match parser
        self.mhl_match_data = parsers.match.HockeyMHLMatchParser(
            html=True,
        ...
        self.mhl2_match_data = parsers.match.HockeyMHL2MatchParser(
            html=True,
            absolute_url='http://mhl2.khl.ru/report/274/?idgame=45340'
        ).get_page(self.mhl2_match_id)
        self.assertIsNotNone
        (self.mhl2_match_data)
        # test khl match parser
        self.khl_match_data = parsers.match.HockeyKHLMatchParser(
            html=True,
        ...
        self.vhl_match_data = parsers.match.HockeyVHLMatchParser(
            html=True,
    ...
    def _create_club(self):
        ...
        # check fields
        for field in ('ru_title', 'html_body', 'url', 'site', 'contacts', 'title'):
    ...
    def _create_schedulers(self):
        ...
        _parsers = (
            #(parsers.schedule.KHLScheduleParser, 266),
